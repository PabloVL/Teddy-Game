...
namespace GameProject
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        protected override void Update(GameTime gameTime)
        {
                    ...
                    if(bears[i].Active && bears[j].Active)
                    {
                        ...
                        if (collisionInfo != null)
                        {
                            ...
                            else
                            {
                                ...
                                //it is not necessary to play teddyBound sound again
                            }
            ...
            foreach (TeddyBear bear in bears)
            {
                if(bear.Active && bear.CollisionRectangle.Intersects(burger.CollisionRectangle))
                {
                    ...
                    //play burgerDamage sound
                    burgerDamage.Play();

                    //update healthString variable
                    healthString = GameConstants.HEALTH_PREFIX + burger.Health;

                    bear.Active = false;

                    //Add new explosion to list
                    explosions.Add(new Explosion(explosionSpriteStrip, bear.Location.X, bear.Location.Y, explosion));

                    //call CheckBurgerKill
                    CheckBurgerKill();
                }
            }
            ...
            foreach(TeddyBear bear in bears)
            {
                foreach (Projectile projectile in projectiles)
                {
                    if(projectile.Type == ProjectileType.FrenchFries && projectile.Active && 
                        bear.Active && projectile.CollisionRectangle.Intersects(bear.CollisionRectangle))
                    {
                        projectile.Active = false;
                        bear.Active = false;
        ...
        private void SpawnBear()
        {
            ...
            while (!CollisionUtils.IsCollisionFree(newBear.DrawRectangle, rectangles))
            {
